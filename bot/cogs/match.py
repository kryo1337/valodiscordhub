...
from utils.db import (
    create_match as create_match_db,
    get_next_match_id,
    calculate_mmr_points,
)
from utils.db import (
...
class AFKCheckView(discord.ui.View):
    ...
    @discord.ui.button(
        label="🚀 Force Start Voting (Testing)", style=discord.ButtonStyle.success
    )
            ...
            if self.match_cog and self.match_cog.bot:
                ...
                if admin_cog:
                    try:
                        ...
                        await admin_cog.cleanup_match_channels(
                            self.message.guild, self.match_id
                        )
...
class CaptainVotingView(discord.ui.View):
    ...
    @discord.ui.button(
        label="2 Highest Rated Players", style=discord.ButtonStyle.primary, emoji="⭐"
    )
    async def vote_highest(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
    ...
    @discord.ui.button(
        label="Random Players", style=discord.ButtonStyle.secondary, emoji="🎲"
    )
    async def vote_random(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        user_id = str(interaction.user.id)
    ...
    async def update_voting_message(self):
        ...
        embed.add_field(
            ...
            + (
                "\n".join([f"• <@{uid}>" for uid in self.votes_highest])
                if self.votes_highest
                else "No votes yet"
            ),
            inline=True,
        )
        ...
        embed.add_field(
            ...
            + (
                "\n".join([f"• <@{uid}>" for uid in self.votes_random])
                if self.votes_random
                else "No votes yet"
            ),
            inline=True,
        )
        ...
        non_voters = [
            p.discord_id for p in self.players if p.discord_id not in voted_users
        ]
    ...
    async def get_highest_rated_captains(self):
        ...
        sorted_leaderboard_players = sorted(
            leaderboard_players, key=lambda x: x.points, reverse=True
        )

        if len(sorted_leaderboard_players) >= 2:
            return [
                sorted_leaderboard_players[0].discord_id,
                sorted_leaderboard_players[1].discord_id,
            ]
        else:
    ...
    async def start_team_selection(self):
        ...
        embed = discord.Embed(title="Team Selection", color=discord.Color.dark_theme())
...
class TeamSelectionView(discord.ui.View):
    ...
    async def update_selection_message(self, message: discord.Message):
        ...
        progress = "▰" * (self.current_selection_index + 1) + "▱" * (
            self.total_picks - self.current_selection_index - 1
        )
        ...
        embed.add_field(
            name="Current Turn", value=f"<@{current_captain}>", inline=False
        )
            ...
            select_menu = discord.ui.Select(
                ...
                options=[
                    discord.SelectOption(
                        label=message.guild.get_member(int(p.discord_id)).display_name
            ...
            async def select_callback(interaction: discord.Interaction):
                if str(interaction.user.id) != current_captain:
                    if str(interaction.user.id) in self.captains:
                        ...
                        await interaction.response.send_message(
                            "❌ You are not a captain! Only captains can select players.",
                            ephemeral=True,
                        )
                    return
        ...
        if len(available_players) == 1:
            ...
            if self.picks_remaining_for_current_captain == 0:
                ...
                if self.current_pattern_index < len(self.selection_pattern):
                    (
                        self.current_captain_index,
                        self.picks_remaining_for_current_captain,
                    ) = self.selection_pattern[self.current_pattern_index]
    ...
    async def on_timeout(self):
        ...
        if self.current_captain_index == 0:
            if (
                selected_player.discord_id not in self.red_team
                and selected_player.discord_id not in self.blue_team
            ):
    ...
    async def end_selection(self, channel: discord.abc.Messageable):
        ...
        lobby_vc = discord.utils.get(
            guild.voice_channels, name="Lobby", category=match_category
        )
        if lobby_vc:
        ...
        if self.selection_message:
            ...
            await channel.send(
                "Error: Selection message not found. Proceeding with new message."
            )
...
class MapBanningView(discord.ui.View):
    ...
    async def update_message(self, message: discord.Message):
        ...
        embed = discord.Embed(title="Map Banning", color=discord.Color.dark_theme())

        progress = "▰" * (self.bans_completed + 1) + "▱" * (
            self.max_bans - self.bans_completed - 1
        )
        embed.add_field(
            name="Progress",
        ...
        if available_maps and self.bans_completed < self.max_bans:
            for i, map_name in enumerate(available_maps):
                ...
                async def ban_callback(
                    interaction: discord.Interaction, map_to_ban=map_name
                ):
                    if str(interaction.user.id) != current_captain:
                        if str(interaction.user.id) in self.captains:
                            await interaction.response.send_message(
                            ...
                            await interaction.response.send_message(
                                "❌ You are not a captain! Only captains can ban maps.",
                                ephemeral=True,
                            )
                        return
...
class SideSelectionView(discord.ui.View):
    ...
    async def attack_callback(self, interaction: discord.Interaction):
        if str(interaction.user.id) != self.side_selector_id:
            await interaction.response.send_message(
                "Only the team captain can select sides!", ephemeral=True
            )
            return
        if self.timeout_task:
            self.timeout_task.cancel()
    ...
    async def update_message(self, message: discord.Message):
        ...
        red_status = (
            f"⚔️ Attack"
            if self.red_side == "attack"
            else f"🛡️ Defense"
            if self.red_side == "defense"
            else ""
        )
        blue_status = (
            f"⚔️ Attack"
            if self.blue_side == "attack"
            else f"🛡️ Defense"
            if self.blue_side == "defense"
            else ""
        )
        ...
        embed.add_field(
            name="🎮 Lobby Master", value=f"<@{match.lobby_master}>", inline=False
        )
        ...
        if not self.red_side and not self.blue_side:
            attack_button = discord.ui.Button(
                label="Attack", style=discord.ButtonStyle.success, emoji="⚔️"
            )
            defense_button = discord.ui.Button(
                label="Defense", style=discord.ButtonStyle.danger, emoji="🛡️"
            )
    ...
    async def check_sides(
        self, interaction: Union[discord.Interaction, discord.Message]
    ):
...
class ScoreSubmissionView(discord.ui.View):
    ...
    async def submit_red_score(
        ...
        if current_time - self.last_submission_time < 5:
            await interaction.response.send_message(
                "Please wait 5 seconds between submissions!", ephemeral=True
            )
            return
        ...
        if not self.score_submission_enabled:
            ...
            if remaining_time > 0:
                ...
                await interaction.response.send_message(
                    f"Score submission will be enabled in {minutes}:{seconds:02d}",
                    ephemeral=True,
                )
                return
        ...
        if str(interaction.user.id) != self.red_captain:
            await interaction.response.send_message(
                "Only the Red Team captain can submit the score!", ephemeral=True
            )
            return

        if self.red_score is not None:
            await interaction.response.send_message(
                "Red Team score has already been submitted!", ephemeral=True
            )
            return
    ...
    async def submit_blue_score(
        ...
        if str(interaction.user.id) != self.blue_captain:
            await interaction.response.send_message(
                "Only the Blue Team captain can submit the score!", ephemeral=True
            )
            return

        if self.blue_score is not None:
            await interaction.response.send_message(
                "Blue Team score has already been submitted!", ephemeral=True
            )
            return
    ...
    async def call_admin(
        ...
        if self.admin_called:
            await interaction.response.send_message(
                "Admin has already been called for this match!", ephemeral=True
            )
            return
        ...
        if not admin_cog or not admin_cog.admin_channel_id:
            await interaction.response.send_message(
                "Admin channel not set up!", ephemeral=True
            )
            return
        ...
        embed.add_field(
            name="Requested by", value=f"<@{interaction.user.id}>", inline=False
        )

        message = await admin_cog.send_admin_report(
            self.match_id, interaction.channel_id, embed
        )
        if message:
            ...
            if self.message:
                ...
                await self.message.edit(view=self)
            await interaction.response.send_message(
                f"⚠️ Admin has been called by <@{interaction.user.id}>.", ephemeral=False
            )
        else:
            await interaction.response.send_message(
                "Failed to send admin report!", ephemeral=True
            )
    ...
    async def update_message(self, message: discord.Message):
        ...
        current_time = time.time()
        if (
            not self.score_submission_enabled
            and (current_time - self.start_time) >= 300
        ):
        ...
        (
            ...
            red_avg,
            blue_avg,
        ) = await self.get_mmr_points_with_averages()
        ...
        embed.add_field(name="🔴 Red Team", value=red_team_value, inline=True)
        embed.add_field(name="🔵 Blue Team", value=blue_team_value, inline=True)
        ...
        if not self.score_submission_enabled:
            ...
            embed.add_field(
                name="Status", value="✅ Score submission is now enabled!", inline=False
            )
    ...
    async def update_leaderboard_points(
        ...
        red_won = winner == "red"
        red_points_change, blue_points_change = calculate_mmr_points(
            red_avg, blue_avg, red_won
        )
    ...
    async def check_scores(self, interaction: discord.Interaction):
        if self.red_score is not None and self.blue_score is not None:
            if (
                not self.validate_score(self.red_score[0])
                or not self.validate_score(self.red_score[1])
                or not self.validate_score(self.blue_score[0])
                or not self.validate_score(self.blue_score[1])
            ):
                embed = discord.Embed(
                    title="Invalid Score",
            ...
            if self.red_score != self.blue_score:
                ...
                if admin_cog and admin_cog.admin_channel_id:
                    ...
                    await admin_cog.send_admin_report(
                        self.match_id, interaction.channel_id, embed
                    )
...
class ScoreModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if not self.view.validate_score(team_score) or not self.view.validate_score(
                opponent_score
            ):
                embed = discord.Embed(
                    title="Invalid Score",
...
async def create_match(
    guild: discord.Guild, rank_group: str, players: List[QueueEntry], bot=None
):
    ...
    match_vc = await match_category.create_voice_channel(
        name="Lobby", user_limit=10, overwrites=overwrites
    )
...
class MatchCog(commands.Cog):
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        for match_id, afk_view in list(self.afk_checks.items()):
            ...
            if str(member.id) in player_ids:
                ...
                left_lobby = (
                    before.channel == afk_view.lobby_vc
                    and after.channel != afk_view.lobby_vc
                )
    ...
    async def on_match_created_from_api(self, guild: discord.Guild, event: dict):
        """
        Handle match created from frontend/API.

        Creates Discord channels and starts the match flow when a match
        is created via the web frontend instead of Discord.
        """
    ...
    async def on_match_updated_from_api(self, guild: discord.Guild, event: dict):
        """
        Handle match updated from frontend/API.

        Updates Discord match channel when match details are changed
        via the web frontend (team changes, draft updates, etc.).
        """
        match_id = event.get("match_id")
        update_type = event.get("update_type")
        data = event.get("data", {})

        # Find the match category and channel
        match_category = discord.utils.get(guild.categories, name=match_id)
        if not match_category:
            return

        match_channel = discord.utils.get(match_category.channels, name="match")
        ...
        if update_type == "cancelled":
            embed = discord.Embed(
                title="Match Cancelled",
                description="This match has been cancelled via the web interface.",
                color=discord.Color.red(),
            )
            await match_channel.send(embed=embed)

    async def on_match_result_from_api(self, guild: discord.Guild, event: dict):
        """
        Handle match result from frontend/API.

        Updates Discord match channel when match result is submitted
        ...
        if result == "cancelled":
            ...
            winner = "RED" if result == "red" else "BLUE"
            embed = discord.Embed(
                title="Match Complete!",
                color=discord.Color.green(),
                description=(
                    "**Final Score (from Web)**\n"
                    f"🔴 Red Team: {red_score}\n"
                    f"🔵 Blue Team: {blue_score}\n\n"
                    f"**{winner} TEAM WINS!**"
                ),
